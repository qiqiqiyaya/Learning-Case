namespace TopologyAlgorithm.DouBao_4
{
    using System;
        ...
        private static bool DFS(int node, Dictionary<int, List<int>> graph, Dictionary<int, NodeState> states, Stack<int> stack, ref List<int> cycle)
        {
            ...
            if (graph.ContainsKey(node))
            {
                foreach (var neighbor in graph[node])
                {
                    ...
                    {
                        // 发现环，回溯找出环的具体信息
                        int cycleStartIndex = cycle.IndexOf(neighbor);
                        cycle = cycle.GetRange(cycleStartIndex, cycle.Count - cycleStartIndex);
                        return false;
                    }
